...
class JogoWidget(BoxLayout):
    ...
    def extrair_arquivos(self):
        """Extrai todos os arquivos .zip na pasta 'jogos'."""
        pasta_jogos = os.path.join("jogos", self.selected_game)
        for root, _, files in os.walk(pasta_jogos):
            for file in files:
                if file.endswith(".zip"):
                    ...
                    try:
                        ...
                        self.status_label.text = f"Arquivo {file} extraído com sucesso."
                    except zipfile.BadZipFile:
                        self.show_popup("Erro", f"Falha ao extrair {file}.")
